...
const provider = {
    ...
    setProvider: async function () {
        const portis = await new Portis('42dca739-f49f-4002-a181-82cdaadc7dd5', 'rinkeby');
    ...
    callTransaction: async function (method) {
        try {
            const transaction = {
                from: this.account,
                to: this.contractAddress,
            }
            const result = await this.contract.methods[method].call(transaction)
            return result;
        } catch (error) {
            console.log(error);
    ...
    sendTransaction: async function (signedTransaction) {
        try {
            const receipt = await this.w3.eth.sendSignedTransaction(
