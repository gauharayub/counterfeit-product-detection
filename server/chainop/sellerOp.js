...
const sellerOp = {
    ...
    async sell(productId, buyerAddress, privateKey) {
        try {
            const signedTransaction = await common.signTransaction(`sellProduct(${productId}, ${buyerAddress})`, privateKey)
    ...
    async register(privateKey, name, details) {
        try {
            const signedTransaction = await common.signTransaction(`registerSeller (${name}, ${details})`, privateKey)
            const result = await common.sendTransaction(signedTransaction)
            return result
        } catch (error) {
            console.log(error.message)
